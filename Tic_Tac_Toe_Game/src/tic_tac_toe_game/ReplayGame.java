...
public class ReplayGame extends BorderPane {
    ...
    public ReplayGame(Stage stage, File file) {
    ...
    private void playGame(Label[] board, File file) {
        thread = new Thread(() -> {
            

            String text = Records.open(file);
            JSONObject textObject = new JSONObject(text);
            JSONArray sequenceArray = textObject.getJSONArray("sequence");
            JSONArray movesArray = textObject.getJSONArray("moves");

            for (int i = 0; i < sequenceArray.length(); i++) {
                try {
                    Thread.sleep(2000);
                } catch (InterruptedException ex) {
                    Logger.getLogger(ReplayGame.class.getName()).log(Level.SEVERE, null, ex);
                }
                index = Integer.parseInt(sequenceArray.getString(i));
                Platform.runLater(() -> {
                    board[index].setText(movesArray.getString(index));
                    System.out.println(movesArray.getString(index) + "in label " + board[index]);
                });

            }
            Platform.runLater(() -> {
                Parent root = new RequestPage(stage);
                stage.setScene(new Scene(root));
                stage.show();
                //new Alert(Alert.AlertType.INFORMATION, "End of Game").show();
            });
